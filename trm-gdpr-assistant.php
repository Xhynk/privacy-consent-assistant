<?php
	...
	*/
	...
	class TRM_GDPR {
			...
			foreach( $policy_pages as $label => $atts ){
				if( $post->ID == $atts['ID'] ){
					if( $post->post_content == '' || $post->post_content == ' ' ){
						$content = file_get_contents( plugin_dir_path( __FILE__ ) . "policies/$label.txt" );

						$content = '<div style="font-size: 14px !important;">'. stripslashes( $this->replace_policy_variables( $content ) ) .'</div>';
					}
		...
		public function replace_policy_variables( $content ){
			// Site URL
