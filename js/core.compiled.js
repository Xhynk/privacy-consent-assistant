...
// Remove Consent Bar
var consentBar = document.querySelector("#trm-gdpr-consent-bar");
var consentVersions = document.querySelector("#trm-gdpr-versions");
...
function closeTRMGDPRconsent() {
  ...
  cookie.value =
    '{"close":true,"versions":"' + versions + '","revised":"' + revised + '"}';
...
(function() {
  var consentCookie = getCookie("trm-privacy-consent");
  ...
  if (consentCookie != null) {
    ...
    if (consentCookie == expectedValue) {
      ...
      if (consentBar != null) consentBar.remove();
      console.log(
        "Privacy Consent Found: [Versions: " +
          versions.replace(/-/g, ", ") +
          "] [Revised: " +
          revised.replace(/-/g, ", ").replace(/_/g, " ") +
          "]"
      );
    } else {
  ...
  for (var i = 0, n = dynamicDelete.length; i < n; ++i) {
    dynamicDelete[i].onclick = function() {
      if (confirm('Click "OK" to remove this notice.')) {
        ...
        var xhrURL =
          url +
          "?trm_gdpr_method=dynamic-delete&dynamic-page=" +
          page +
          "&dynamic-index=" +
          index +
          "&nonce=" +
        ...
        if (formID != null) xhrURL = xhrURL + "&form_id=" + formID;
